#!/usr/bin/env python3
"""Run the Norx userdb atomic-update/reload smoke through nsh and QEMU."""
...
COMMANDS = (
    (
        ...
        b"norx:/$ ",
        b"userdb-smoke\n",
        b"userdb: atomic update and reload",
        None,
    ),
    ("marker", b"norx:/$ ", b"echo USERDB:OK\n", b"USERDB:OK", None),
...
def main() -> int:
    ...
    command_index = 0
    failure = None
    deadline = time.monotonic() + float(environment.get("NORX_USERDB_SMOKE_TIMEOUT", "150"))
    try:
        ...
        process.wait()
    if failure:
        print(f"userdb smoke response check failed: {failure}", file=sys.stderr)
        sys.stdout.buffer.write(output)
        return 1
    if command_index != len(COMMANDS):
        print(f"userdb smoke stopped before command {command_index + 1}/{len(COMMANDS)}", file=sys.stderr)
        sys.stdout.buffer.write(output)
        return 1
    missing = [marker for marker in REQUIRED_OUTPUT if marker not in output]
    if missing:
        print(f"userdb smoke missing markers: {missing}", file=sys.stderr)
        sys.stdout.buffer.write(output)
        return 1
    sys.stdout.buffer.write(output)
    print(f"userdb {arch} smoke passed; serial halt reached")
    return 124
