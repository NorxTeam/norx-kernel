...
const DATA_SECTORS: usize = RAMDISK_SECTORS;
...
pub fn init() -> bool {
    ...
    unsafe {
        ...
        for i in 0..SECTOR_SIZE * DATA_SECTORS {
    ...
    discover_partitions();
    if !smoke_test() {
        crate::bootlog::fail("ramdisk block queue self-check failed");
        return false;
    }
    let geometry = geometry();
    crate::bootlog::ok_fmt(format_args!(
        "ramdisk geometry sector={} sectors={} queue={} partitions={} readonly={} cache={}",
...
pub fn poll() {
    for index in 0..QUEUE_DEPTH {
        ...
        process(request);
        unsafe {
            (&mut *core::ptr::addr_of_mut!(QUEUE))[index].state = RequestState::Complete;
...
fn submit<'a>(
    ...
    if lba
        .checked_add(sectors)
        .is_none_or(|end| end > DATA_SECTORS as u64)
    {
...
fn process(request: Request) {
    ...
    unsafe {
        ...
        match request.operation {
            Operation::Read => ptr::copy_nonoverlapping(disk, request.buffer, request.length),
            Operation::Write => ptr::copy_nonoverlapping(request.buffer, disk, request.length),
        }
    }
}

fn smoke_test() -> bool {
    let lba = (DATA_SECTORS - 1) as u64;
...
fn write_mbr(disk: *mut u8) {
    unsafe {
        ...
        write_le_u32(entry.add(12), (DATA_SECTORS - 1) as u32);
...
fn discover_partitions() {
    ...
    unsafe {
        ...
        ptr::copy_nonoverlapping(disk, sector.as_mut_ptr(), SECTOR_SIZE);
    }
    let mut count = 0;
    if sector[510] == 0x55 && sector[511] == 0xaa {
        for index in 0..MAX_PARTITIONS {
            ...
            if type_code == 0
                || sectors == 0
                || start_lba >= DATA_SECTORS as u64
                || sectors > DATA_SECTORS as u64 - start_lba
            {
                continue;
            }
            unsafe {
