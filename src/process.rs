...
pub const BOA_EXEC_MAGIC: u32 = 0x5845_4f42;
pub const BOA_EXEC_ABI: u16 = 1;

#[derive(Clone, Copy)]
pub struct BoaExecutable {
    pub magic: u32,
    pub abi: u16,
    pub entry: u16,
    pub flags: u32,
...
pub fn list(mut f: impl FnMut(&'static str, BoaExecutable, crate::capability::Set)) {
...
fn valid_object(object: BoaExecutable) -> bool {
    object.magic == BOA_EXEC_MAGIC
        && object.abi == BOA_EXEC_ABI
        && object.entry != 0
        && object.code_len <= 491
}

fn load_object(path: &str) -> Option<BoaExecutable> {
    Some(load_object_with_code(path)?.0)
}

fn load_object_with_code(path: &str) -> Option<(BoaExecutable, &'static [u8])> {
    let mut bytes = [0u8; 511];
    let len = crate::vfs::read(path, &mut bytes)?;
    if len < 20 {
        return None;
    }
    let object = BoaExecutable {
...
fn hello(args: &[&str]) -> i32 {
    crate::kprint!("hello from Boa process");
    for arg in args {
        crate::kprint!(" {}", arg);
    }
    crate::kprintln!();
