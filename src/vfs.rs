...
fn write_object(lba: usize, entry: u16, caps: crate::capability::Set) {
    ...
    let len = 20 + code.len();
    sector[0] = len as u8;
    sector[1..5].copy_from_slice(&crate::process::BOA_EXEC_MAGIC.to_le_bytes());
    sector[5..7].copy_from_slice(&crate::process::BOA_EXEC_ABI.to_le_bytes());
