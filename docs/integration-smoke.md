...
The aarch64 timer gate also emits `NORX_AARCH64_GIC_TIMER_IRQ_OK v=1` only
after the DTB-discovered GIC has acknowledged at least eight generic-timer
interrupts, completed EOI, and advanced scheduler accounting. Discovery or
accounting failures emit the versioned `NORX_AARCH64_GIC_TIMER_IRQ_FAIL`
marker instead of being reported as a false positive.

The default AArch64 `virt` run covers GICv2. A GICv3 pass uses a matching
machine/DTB pair, for example `QEMU_MACHINE=virt,gic-version=3,its=off` with
the DTB generated by that same machine profile.
