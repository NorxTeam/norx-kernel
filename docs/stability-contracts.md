...
| Block and filesystems | Block layer owns request buffers/queue completion; each filesystem owns its parsed superblock, mounts, and file handles. x86_64 may publish one modern PCI virtio-blk backend with a bounded polling queue and flush capability; otherwise the volatile RAM disk remains the fallback. Probed filesystems consume an explicit bounded partition view. | Invalid request, timeout, read-only write, malformed metadata, used-ring corruption, backend failure, and unsupported feature return typed errors; an initialized virtio device is never silently replaced by RAM storage. | Sector geometry, `RequestId<'a>`, partition slice, cache, backend status, VFS mount, read-only filesystem, and fixed-file persistence contracts. | RAM block smoke and FAT32/ext4/btrfs fixtures on both targets; CI boots the same bounded FAT32 image twice on x86_64 and checks `/NORX.PST` sequence readback. | FAT32 free-space allocation, full mount-tree integration, ext4/btrfs writes and journal recovery, page cache, and general production media remain deferred. |
| VFS and file handles | VFS owns mount tree, namespace, dentry/inode references, file offsets, and unmount validation; process tables own bounded descriptor references. | Invalid/generation-stale handles, permissions, type-aware path traversal, read-only mounts, mountpoint mutation, duplicate targets, and unsafe unmounts return errors without leaked references. | Internal VFS API plus bounded process-local `open`/`pipe`/`dup2`/`close`/`mkdir`/`rmdir`/`unlink`/`rename`/`link`/`stat`/`fstat`/`read_dir`/`lseek`/`fchmod`; `/hello.txt` and `/lib` remain fixture paths, not a host-filesystem ABI. | In-kernel ramfs with bounded mount, namespace, descriptor, metadata, hard-link, and directory lifecycle smoke; CI asserts `/hello.txt` readback on x86_64 and aarch64. | Process-attached mount namespaces, persistent storage namespaces, symlink nodes, and a complete POSIX filesystem ABI are deferred. |
