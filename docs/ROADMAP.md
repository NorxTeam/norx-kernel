...
- User program launches now allocate a PID and record state, exit code, capabilities, path, and code size in a small fixed process table exposed by `ps` and `procs`.
