...
| Logging and console | `drivers::serial` owns UART I/O; `log` fans output to serial, VGA, and an optional firmware framebuffer. | Bounded UART writes mark the serial path failed; framebuffer loss keeps serial diagnostics active. | ASCII serial stream; boot markers and `NORX_SERIAL_DEBUGGER_READY v=1` are smoke interfaces. | x86_64 NS16550 COM1, x86 VGA/virtio-GOP, and aarch64 PL011 on QEMU `virt`. | No framebuffer keyboard shell; ARM QEMU may have no GOP and therefore uses serial-only fallback. |
...
| Memory and paging | `memory` owns the static physical range table and frame allocation; architecture paging owns page tables and mappings. | Exhaustion returns `None` or a boot failure; invalid mappings, W+X, and fault addresses are rejected. | Internal page/address types; no userspace mapping ABI is frozen beyond the documented address-space contract. | x86_64 direct map, CR3 switching, and lazy-fault path; aarch64 table setup and TTBR0 boundary checks. | aarch64 lazy fault mapping, general mapping teardown, ASLR entropy, and IOMMU-backed allocation remain deferred. |
| IRQ, timer, and scheduler | Architecture code owns interrupt delivery; deferred work is drained by the normal-context poller; scheduler owns task accounting. | Unavailable hardware timer falls back to bounded polling; interrupt storms are coalesced and accounted. | Internal IRQ registration and scheduler status structures; no driver may run unbounded hard-IRQ work. | x86 legacy PIC/PIT/APIC inspection under QEMU; aarch64 counter polling under `virt`. | Final x86 APIC/HPET timer routing, aarch64 GIC/generic-timer IRQ delivery, MSI/MSI-X, and threaded IRQs are deferred. |
...
| Display | Boot `RawFramebuffer` is reserved by `boot`; `display` owns mode/status/damage state and never frees firmware memory. | Invalid mode or damage returns a typed error; absent framebuffer leaves serial active; unsupported EDID/hotplug is explicit. | Framebuffer descriptor, mode, damage, cursor, EDID, and hotplug contracts. | Firmware framebuffer and basic 2D virtio-gpu on x86_64 QEMU with `virtio-vga`. | 3D acceleration, firmware mode changes, EDID parsing, hotplug events, and aarch64 virtio-gpu are deferred. |
...
| VFS and file handles | VFS owns mount tree, namespace, dentry/inode references, file offsets, and unmount validation. | Invalid handles, permissions, path traversal, read-only mounts, and unsafe unmounts return errors without leaked references. | Internal VFS API; `/hello.txt`, `/lib`, and `/lib64` are current fixture paths, not a host-filesystem ABI. | In-kernel ramfs with bounded mount and namespace smoke. | Persistent storage namespaces, process-visible descriptor inheritance, and a complete POSIX filesystem ABI are deferred. |
...
| Syscalls and usercopy | Syscall entry owns register decoding; `usercopy` validates ranges; process/address-space code owns user mappings. | Unknown calls return `-ENOSYS`; invalid pointers/ranges return negative errno; no unchecked user pointer is dereferenced. | Versioned ABI v1, Linux-shaped numbers, six-word args, 64-bit types, negative errno returns, and bounded stdio `read/write`. | x86_64 `syscall/sysret` and aarch64 SVC boundary, including real user-buffer console I/O, self-check under graphical QEMU. | Filesystem-backed descriptors, blocking streams, fork/exec compatibility, signals, and a glibc ABI are deferred. |
