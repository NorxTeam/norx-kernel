...
| Syscalls and usercopy | Syscall entry owns register decoding; `usercopy` validates ranges; process/address-space code owns user mappings. | Unknown calls return `-ENOSYS`; invalid pointers/ranges return negative errno; no unchecked user pointer is dereferenced. | Versioned ABI v1, Linux-shaped numbers, six-word args, 64-bit types, and negative errno returns. | x86_64 `syscall/sysret` and aarch64 SVC boundary self-checks under QEMU. | `read`/`write` against real process pages, fork/exec compatibility, signals, and a glibc ABI are deferred. |
