#!/usr/bin/env python3
"""Run the Norx getty prompt, timeout/retry, and login-handoff smoke."""
...
COMMANDS = (
    ("launch", b"norx:/$ ", b"getty-smoke\n", b"GETTY:READY", None),
    (
        "handoff",
        b"GETTY:PROMPT norx login: ",
        b"alice\n",
        b"GETTY:HANDOFF login=/bin/login",
        None,
    ),
    (
        "exit",
...
def main() -> int:
    ...
    environment.update(
        ...
        QEMU_DISPLAY=os.environ.get("QEMU_DISPLAY", "none"),
    )
    if os.name == "nt":
        try:
    ...
    command_index = 0
    failure = None
    deadline = time.monotonic() + float(environment.get("NORX_GETTY_SMOKE_TIMEOUT", "150"))
    try:
        ...
        process.wait()
    if failure:
        print(f"getty smoke response check failed: {failure}", file=sys.stderr)
        sys.stdout.buffer.write(output)
        return 1
    if command_index != len(COMMANDS):
        print(f"getty smoke stopped before command {command_index + 1}/{len(COMMANDS)}", file=sys.stderr)
        sys.stdout.buffer.write(output)
        return 1
    missing = [marker for marker in REQUIRED_OUTPUT if marker not in output]
    if missing:
        print(f"getty smoke missing markers: {missing}", file=sys.stderr)
        sys.stdout.buffer.write(output)
        return 1
    sys.stdout.buffer.write(output)
    print(f"getty {arch} smoke passed; serial halt reached")
    return 124
